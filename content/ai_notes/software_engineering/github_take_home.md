...
This has been my general approach in using the CAP theorem to address tradeoffs in database, 
but it has got me thinking lately as to whether the Venn diagram used to depict this "theorem" 
is really accurate. RDBM systems are touted as achieving both consistency and availability while sacrificing
partition tolerance, but is that really true? What was Dr. Eric Brewer really trying 
to say when he presented CAP theorem in 2000? I turned to Martin Klepperman 
*Please Stop Calling Databases CP or AP* and Coda Hale's *You Can't Sacrifice Partition Tolerance* 
for answers, and this is what I learned. 
...
as our distributed systems are already working embedded in contexts with some partition tolerance.
Hale puts this more directly in stating that "for a distributed system to not require
...
Take "availability" for instance. In the context of CAP, and available system is 
defined as "every request received by a non-failing node in the system must result
in a non-error response". This is a very view on avaiability, and says nothing of 
nodes that have crashed or are being recovered, software bugs, or databases that have
run out of space. The point being, there is a range of "availability" decisions 
that extend beyond the scope of a non-failing node's response. 

Turning to consistency, Hale reminds us that standard data replication is not "strongly
consistent" as there is inherent lag, which we must explicitly address. Instaneous
and global consistency in a distributed system is impossible, given the physical laws
of the universe, but in practice we typically mean a reduction of lag to a level that
is imperceptible. 
...
We can assume our frontend is running on clusters in each of our service availability zones, 
with load balancers effectively handling traffic to our processing layer. We can implement a CDN 
to cache all commonly requested content in these regions, with Least Recently Used (LRU) eviction strategy,
as well as a cache layer behind our servers to support common database lookups and processing requests. 
...
to our database cluster will be processed by our master nodes and written and replicated 
to our replicas. Reads can happen from any node. MySQL Workbench will be set up for DB monitoring. 
We will enable our slow-log by setting the appropriate parameters in the $my.cng$ file. 
...
The steps for login will be, genarically, if the user is in the user database
...
So, the login API will be something like
```
LOGIN(timestamp, IP, user, password)
```

and will send to our verification processor a JSON file
```
...
Our verification processor will look something like the following:
...
I'm going to use Yelp's traffic as an example, as their persistence
...
impacted by slow login, especially during peak hours, and similarly, any
...
If we find there is a natural pattern of increased login that leads to bottlenecks, 
how can our load balancers and MySQL cluster partitioning be redesigned to handle such traffic?
